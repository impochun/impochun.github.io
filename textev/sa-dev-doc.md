...
若會員在前台修改會員資料，修改完成後，要打EPB VIP Update API去給EPB同步修改EPB的會員資料。
...
每次會員登入後，就打API去EPB取得最新的會員等級、會籍到期日、累積消費金額、目前總點數資料，更新至該會員檔案中。

若會員持續處於登入狀態，都沒登出，則在隔日會員初次瀏覽EC(APP或PC)時，打API更新上述資料 (這個做法若不是最好的可以討論怎麼做更好)。

|會員欄位名稱|來源                 |備註 |
|------|-------------------|---|
|* EC會員編號|EC |EC系統產生的會員唯一值 |
|VIP No(VIP代碼) |EPB                | 來自EPB的會員唯一值，不可修改 |
|手機    |EPB/EC             | 會員無權修改，管理者後台可修改 |
...
|(預設地址) 地區    |EC                 |   |
|(預設地址) 詳細地址    |EC                  |   |
|會員等級  |EPB                | EC無法修改，會員無權修改，管理者可於EPB修改 |
|累積消費金額|EPB                | EC無法修改，會員無權修改，管理者可於EPB修改 |
|會籍到期日 |EPB                | EC無法修改，會員無權修改，管理者可於EPB修改 |
|目前總點數 |EPB | EC無法修改，會員無權修改，管理者可於EPB修改 |
|註冊推薦碼  |EC                 | 英文+數字 |
...
**組合商品庫存維護**：維護一個EC商品庫存，不維護各個EPB存代庫存，各個存代庫存由StudioA自行控管。

**組合商品價格為**：EPB原價-折扣，整個組合商品會對應一個折扣&折扣存代。

透過訂單(銷售)API拋回EPB：若訂單中包含組合商品，拋回EPB的訂單資訊中需要包含原價、折扣及折扣存代。
...
有純點/點加金兩種價格需要設定，結帳時須強制用點數/點加金結帳

1. 純點
2. 點 + 金
3. 下圖範例是錯的，不會有兩種規格，紅利商品僅准一種規格，兩種價格(純點/點+金)
...
- 需可設定序號商品為獨立免運項目(單品結帳不受免運門檻限制皆免運；可與其他商品湊全站免運門檻)
- 序號商品可設定單品免運，但可與其他商品湊滿額。
...
- 系統需記錄補貨時間(上傳csv檔時間)。
- 自動Email出貨時，系統將自動從序號庫存中取用一組序號出貨。
- Email出貨後，將該組序號標記為已出貨、註記出貨時間、訂單編號、會員編號。
- SA管理者/客服可於後台查詢各組序號發送狀態。

| 序號                           | 新增時間            | CSV檔名     | 狀態   | 出貨時間            | 訂購會員 | 訂單編號 |
| ------------------------------ | ------------------- | ----------- | ------ | ------------------- | -------- | -------- |
| QW9JAJC8SDF (舉例, 非真實序號) | 2021-04-01 19:00:00 | KKBOX01.csv | 已出貨 | 2021-04-02 19:05:00 | 12345678 | TM0001   |
| QW8JC89SDN (舉例, 非真實序號)  | 2021-04-01 19:00:00 | KKBOX01.csv | 已出貨 | 2021-04-03 11:22:00 | 23456789 | TM0099   |
| 398J48WEFNS (舉例, 非真實序號) | 2021-04-01 19:00:00 | KKBOX01.csv | 未發送 | 無                  | 無       | 無       |
| QWOIMEIS(舉例, 非真實序號)     | 2021-04-01 19:00:00 | KKBOX01.csv | 未發送 | 無                  | 無       | 無       |
| QWOEOQIWE(舉例, 非真實序號)    | 2021-04-01 19:00:00 | KKBOX01.csv | 未發送 | 無                  | 無       | 無       |
...
- 取券透過宜睿API進行。
- 需可設定序號商品為獨立免運項目(單品結帳不受免運門檻限制皆免運)。
- 購券後接收宜睿通知部份，需開一個API接口給宜睿串接，API開立規格須依照宜睿提供之PDF文件"Notification"這份去製作，key的部份則與宜睿提供的API其餘key使用同一隻key。
...
管理預約單/預約訂單管理，可進行預約排序、產生結帳連結等，請參照下方預約機制說明。

**團購訂單**

團購訂單管理/成團/未成團管理，細節請參照下方團購機制說明。
...
- 下表已過時，有更新正式文件：EPB API: 銷售API/ 銷退API。

| <del>結帳/退貨回傳所需欄位</del>                            | <del>欄位API名稱</del> | <del>Data Type</del> |
| ----------------------------------------------------------- | ---------------------- | -------------------- |
| 物流方式：快送 OR 宅配                                      | ref3                   | VARCHAR2 (512 Byte)  |
| 存貨代碼                                                    | stkId                  | VARCHAR2 (32 Byte)   |
| 主機序號                                                    | srnId                  |                      |
| 訂購數量                                                    | stkQty                 | NUMBER (20,6)        |
| 單價                                                        | listPrice              | NUMBER (20,6)        |
| TM單號(主單號)                                              | remark                 | VARCHAR2 (2000 Byte) |
| TS放在(子單號)                                              | ref1                   | VARCHAR2 (512 Byte)  |
| EC倉庫代碼：固定SA801                                       | storeId                | VARCHAR2 (16 Byte)   |
| EC地點代碼：固定801                                         | locId                  | varchar2(8)          |
| 出貨倉庫：調撥單的出庫倉                                    | ref2                   | VARCHAR2 (16 Byte)   |
| 員工編號                                                    | empId1 (empName1)      | VARCHAR2 (32 Byte)   |
| VIP NO                                                      | vipId                  | VARCHAR2 (64 Byte)   |
| 發票號碼                                                    | taxInvNo               | VARCHAR2 (32 Byte)   |
| 付款方式                                                    | pmId                   | VARCHAR2 (16 Byte)   |
| <del>出貨日期 OR 退貨日期</del> 訂單成立日/退貨申請單成立日 | docDate                | DATE                 |
| 部門：固定放SA801                                           | storeId                | VARCHAR2 (16 Byte)   |
| 促銷活動代碼                                                | mcId                   |                      |
...
| 領取方式     | 官網 / APP (複選)<br />官網則僅能透過網站領取、APP則僅能透過APP領取；複選則表示不限制方式都可領取。 |      |
| 領取期限     | 日期區間                                                     |      |
| 使用期限     | 日期區間 / 與領取期限相同                                    |      |
...
| 適用商品     | (單選)<br />限定分類 / 單品<br />排除分類 / 單品<br />全館適用 |      |
| 適用通路     | 線上 / 門市 (複選)<br />線上則代表使用本系統EC時可用、門市則使用者出示該折價券給門市人員手動刷手機折價券條碼核銷(畫面顯示EPB會員編號條碼+EPB折扣存代條碼+本系統折價券條碼)，EC不與EPB或POS串接核銷。 |      |
| 使用條件     | 不限<br />滿額門檻：設定一滿額門檻金額，訂單需符合此門檻才可使用本券。<br />最高可折抵金額：整張訂單的一定比例% / 固定金額 |      |
| EPB折扣存代  | 自行輸入EPB對應的存代，訂單使用折扣時將帶入此存貨代碼至API回傳EPB |      |
