...
| 圖片       | 贈品圖片                                               |      |
| 特色       | 贈品簡短特色說明                                       |      |
| 庫存       | 可設定數字，倉別有EC倉及各個門市倉<br />可設定無限庫存 |      |
| 上下架時間 | 輸入時間區間                                           |      |
| 啟用狀態   | 開啟/關閉                                              |      |
...
**總部倉庫**：同門市管理介面，但總部倉庫是最後一關(門市都無法出才會轉到總部)，因此不可使用無法接單的選項、且開放使用「黑貓」出貨。

管理介面參考：

![fastbk01](img/fastbk01.png)

![fastbk02](img/fastbk02.png)

![fastbk03](img/fastbk03.png)
...
主題活動修改：主題活動不會自己獨立存在，要跟著促銷活動去建立。
...
10. 通知：
    - 正取通知信件：一但完成付款及發送
    - 備取轉正取通知信件：課程管理者將備取手動加入正取名單時
    - 課程通知信件(提醒上課)：課程前 48 小時
    - 滿意度信件：課程一結束即發送
    - 課程取消通知信件：課程管理者手動按下課程取消時
